...
df = pd.read_csv('Registos_Incendios_SGIF_2011_2020.csv', encoding='ISO-8859-1', nrows=177000)

df.rename(columns={'Ano': 'year', 'Mes': 'month', 'Dia': 'day', 'Hora': 'hour'}, inplace=True)
...
df_fires_extract = df[['Codigo_SGIF', 'datetime', 'AreaTotal_ha', 'DataHoraAlerta', 'DataHora_PrimeiraIntervencao', 'DataHora_Extincao', 'Latitude', 'Longitude', 'DSR', 'FWI', 'ISI', 'DC', 'DMC', 'FFMC', 'BUI', 'FonteAlerta','CodCausa', 'DTCCFR']]
...
df_fires_extract.loc[:, 'datetime'] = pd.to_datetime(df_fires_extract['datetime'], errors='coerce')
df_fires_extract.loc[:, 'DataHoraAlerta'] = pd.to_datetime(df_fires_extract['DataHoraAlerta'], errors='coerce')
df_fires_extract.loc[:, 'DataHora_PrimeiraIntervencao'] = pd.to_datetime(df_fires_extract['DataHora_PrimeiraIntervencao'], errors='coerce')
df_fires_extract.loc[:, 'DataHora_Extincao'] = pd.to_datetime(df_fires_extract['DataHora_Extincao'], errors='coerce')


fires_columns = [
    'code_SGIF', 'date_time', 'total_area_ha', 'TIMESTAMP_alert', 'TIMESTAMP_first_intervention', 
    'TIMESTAMP_extinction', 'latitude', 'longitude', 'DSR', 'FWI', 'ISI', 'DC', 'DMC', 
    'FFMC', 'BUI', 'alert_source', 'cod', 'code_DTCCFR'
]
